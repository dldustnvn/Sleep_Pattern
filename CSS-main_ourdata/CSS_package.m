...
tau_c = 24.09;                                                              %Change according to chronotype of usersm 사용자의 시간 유형에 따른 변경
                                                                            %Morning type = 23.85, evening = 24.35 (h) -> 평균은 24.1, 근데 왜 24.09?
    
% read two input files
disp("---CSS_calculation---")
sleep_light = readtable('Input1_sleep_light_p101.csv');                          %Read input1: sleep-wake pattern (1st column) and light exposure (2nd column)
waso_main = readtable('Input2_WASO_main_p101.csv');                              %Read input2: WASO (1st column) and whether main sleep or not (2nd column)
